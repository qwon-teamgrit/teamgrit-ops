...
async function driveDownload(token,fileId){const r=await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}?alt=media`,{headers:{Authorization:`Bearer ${token}`}});if(!r.ok)throw new Error(`Drive ${r.status}: ${(await r.text()).slice(0,300)}`);return Buffer.from(await r.arrayBuffer())}
async function driveUpload(token,fileId,buf){const r=await fetch(`https://www.googleapis.com/upload/drive/v3/files/${fileId}?uploadType=media`,{method:'PATCH',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'},body:buf});if(!r.ok)throw new Error(`Drive write ${r.status}: ${(await r.text()).slice(0,300)}`);return r.json().catch(()=>({}))}
function parseWorkbook(wb){const master=rows(wb.Sheets['고객 마스터']||{}),actions=rows(wb.Sheets['후속 액션']||{}),distributions=rows(wb.Sheets['배포 이력']||{}),companies=[],contacts=[],followups=[],distributionHistory=[];
